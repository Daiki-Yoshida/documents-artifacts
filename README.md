...
更新: "同名ファイルの内容が変わっていれば上書きする"
維持: "内容が同じファイルと、manifestにない同期先独自ファイルは変更しない"
削除: "前回manifestにあり、現在の配布元からなくなったファイルだけを削除する"
```

初回実行でmanifestが存在しない場合は、既存ファイルを削除しません。現在の配布元ファイルを追加・更新してmanifestを作成し、次回以降の削除範囲を確定します。そのため、旧スクリプトで過去に配布され、すでに配布元から削除されたファイルは、初回のmanaged syncでは自動判定できません。
...
テストは、初回導入、上書き、追加、廃止済み管理ファイルの削除、同期先独自ファイルの保護、入力不足時のpreflight失敗を確認します。
